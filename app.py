...
# --- Fungsi untuk mengambil data dari semua vendor ---
        ...
        if view_mode == "Ringkasan 100g":
            ...
            if not df_table.empty:
                ...
                if st.button("💾 Simpan Ringkasan 100g ke Histori"):
                    if save_to_history(df_table, "Summary_100g"):
                ...
                with st.expander(f"🔍 {v_name} - {lbl_v}"):
                    ...
                    if st.button(f"Simpan Histori Lengkap {v_name}", key=f"save_{v_name}"):
                        ...
                        if save_to_history(df_v, tab_name):
                            st.success(f"✅ Data {v_name} berhasil disimpan!")
    else:
        st.info("Klik 'Tarik Data Sekarang' untuk mengambil harga terbaru.")

# =========================================================
# 5. TAB HISTORI & GRAFIK (DENGAN PERBAIKAN)
# =========================================================
with tab2:
    ...
    # Ambil data histori (pastikan history_manager.py menggunakan ttl=0)
    ...
    if not df_hist.empty:
        ...
        # 2. Filter berat yang tersedia untuk vendor terpilih
        ...
        # 3. Filter data dan URUTKAN berdasarkan waktu untuk garis yang benar
        ...
        if not plot_df.empty:
            ...
            # 4. Format angka sumbu Y agar tidak menggunakan scientific notation
        ...
        with st.expander("📂 Lihat Data Mentah"):
            # Tampilkan data dengan baris terbaru di atas
            st.dataframe(df_hist.sort_values("timestamp", ascending=False), use_container_width=True, hide_index=True)
    else:
        st.info("Belum ada data histori. Silakan simpan data di Tab 1.")
